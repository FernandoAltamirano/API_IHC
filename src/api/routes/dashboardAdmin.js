...
router.get("/myrequests/details/:id", isLoggedIn, isLoggedInAdmin, async (req, res) => {
    ...
    if (rows.length > 0) {
        ...
        res.status(200).json({DetailRequest: rows})
    }
    else{
...
router.put("/myrequests/details/approved/:id", isLoggedIn, isLoggedInAdmin, async (req, res) => {

    const { id } = req.params

    const { observaciones } = req.body

    const query = "UPDATE solicitudes SET estado = 1, observaciones = ? WHERE solicitudes.id = ?"
...
router.put("/myrequests/details/dismissed/:id", isLoggedIn, isLoggedInAdmin, async (req, res) => {

    const { id } = req.params

    const { observaciones } = req.body

    const query = "UPDATE solicitudes SET estado = 0, observaciones = ? WHERE solicitudes.id = ?"
