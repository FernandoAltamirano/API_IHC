...
router.get("/", async (req, res) => {
    ...
    const query1 = "SELECT id, titulo, DATE_FORMAT(fecha_inicio, '%W, %d de %M del %m') AS fecha_inicio, DATE_FORMAT(hora_inicio, '%h %p') AS hora_inicio, precio_inscripcion FROM eventos GROUP BY participantes DESC"

    const query2 = "SELECT id, titulo, DATE_FORMAT(fecha_inicio, '%W, %d de %M del %m') AS fecha_inicio, DATE_FORMAT(hora_inicio, '%h %p') AS hora_inicio, precio_inscripcion FROM eventos WHERE fecha_inicio > NOW() GROUP BY fecha_inicio"
    ...
    if (features.length > 0 && closeOnes.length > 0) {
        ...
        res.status(200).json({featured_events: features, nearby_events: closeOnes})
    }
    else{
...
router.get("/title/:titulo", async (req, res) => {
    ...
    const query = "SELECT id, titulo, DATE_FORMAT(fecha_inicio, '%W, %d de %M del %m') AS fecha_inicio, DATE_FORMAT(hora_inicio, '%h %p') AS hora_inicio, precio_inscripcion FROM eventos WHERE titulo = ?"
...
router.get("/prices/:precio1/:precio2", async (req, res) => {
    ...
    const query = "SELECT id, titulo, DATE_FORMAT(fecha_inicio, '%W, %d de %M del %m') AS fecha_inicio, DATE_FORMAT(hora_inicio, '%h %p') AS hora_inicio, precio_inscripcion FROM eventos WHERE precio_inscripcion BETWEEN ? AND ?"
...
router.get("/category/:categoria", async (req, res) => {
    ...
    const query = "SELECT e.id, e.titulo, DATE_FORMAT(e.fecha_inicio, '%W, %d de %M del %m') AS fecha_inicio, DATE_FORMAT(e.hora_inicio, '%h %p') AS hora_inicio, e.precio_inscripcion FROM eventos AS e JOIN tipos_evento AS te ON e.tipo_evento = te.id WHERE te.id = ?"
...
router.get("/dates/:fecha1/:fecha2", async (req, res) => {
    ...
    const query = "SELECT id, titulo, DATE_FORMAT(fecha_inicio, '%W, %d de %M del %m') AS fecha_inicio, DATE_FORMAT(hora_inicio, '%h %p') AS hora_inicio, precio_inscripcion FROM eventos WHERE fecha_inicio BETWEEN cast(? AS date) AND cast(? as date) GROUP BY fecha_inicio"
